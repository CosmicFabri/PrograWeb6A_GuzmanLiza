...
public class AutenticacionServlet extends HttpServlet {
    ...
    private void register(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        boolean emailExists = users.values().stream().anyMatch(user -> user.getEmail().equals(email));

        if (usernameExists || emailExists) {
            jResp.failed(req, resp, "El nombre de usuario o correo ya están registrados", 422);
            return;
        }
        ...
        // Guardar el nuevo usuario en el HashMap
    ...
    private void login(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        // Verificar si el parámetro es un username o un email
        for (Map.Entry<String, User> entry : users.entrySet()) {
            if (entry.getKey().equals(usernameOrEmail) || entry.getValue().getEmail().equals(usernameOrEmail)) {
                user = entry.getValue();  // Encontró el usuario
                break;
            }
        }
        ...
        if (user != null && verifyPassword(password, user.getPassword())) {
            // Establecer la expiración en 5 minutos
            long expirationTime = System.currentTimeMillis() + (5 * 60 * 1000);  // 5 minutos en milisegundos

            String token = Jwts.builder()
                    .setSubject(user.getUsername())   // Agregar el subject como el username
                    .setExpiration(new Date(expirationTime))  // Expiración
                    .signWith(generalKey())  // Firma con la clave secreta
                    .compact();
